...
public class Graph extends javax.swing.JFrame {
    ...
    public String[] errorPlotLabel = new String[]{"Newton Raphson", "Secant", "Bisection", "Steffensen"};
    ...
    public ArrayList<ArrayList<Variables>> list = new ArrayList<>(); //ArrayList of (ArrayList of Variables)

    public Graph(int option) {
        initComponents();
        this.setExtendedState(JFrame.MAXIMIZED_BOTH); //Maximized

        //adding ArrayList to ArrayList of ArrayLists
        list.add(CourseworkGUI.raphsonData);
        list.add(CourseworkGUI.secantData);
        list.add(CourseworkGUI.bisectionData);
        list.add(CourseworkGUI.steffensenData);
    ...
    /*private void functionPlot() {
		range = CourseworkGUI.graphrange;
	...
	private void createFunctionPlot() {
		...
		for (int num = 0; num < functionNumber; num++) {
			...
			for (double i = graphdata - range; i <= graphdata + range; i += increment) {
				double value;
				if (num == 0) {
					value = i; //get value of f(x) for Y Axis
				} else {
					value = i; //get value of g(x) for Y Axis
				}
    ...
    private void createErrorPlot() {
        ...
        for (int num = 0; num < 4; num++) {
            ...
            ArrayList<Variables> rooDataList = list.get(num); //get rootData list			
    ...
    private void createFxPlot() {
        ...
        for (int num = 0; num < 4; num++) {
            ...
            for (int i = 0; i < rooDataList.size(); i++) {
                int iteration = rooDataList.get(i).i; //get iteration for X Axis
                double x = rooDataList.get(i).x; //get x for Y Axis

                if (Double.isNaN(x) == false) {
                    series[num].getData().add(new Data(iteration, x)); //add Data to Series
                }
            }
